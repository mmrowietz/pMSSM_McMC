...
double phi22(double delta, double z)
{
	...
	t=(1.-delta)/z;
	double int2=pow(1.-z*t,2.)*G1(t)/2.;
...
double phi27(double delta, double z)
{
	...
	t=(1.-delta)/z;
	double int2=pow(1.-z*t,2.)*G2(t)/2.;
...
{
	double mb=param->mass_b+0.37; /* mb(kin) */
	double Lb=log(mu*mu/mb/mb);
	int nf=5;
...
double phi77_2rem(double delta, struct parameters* param)
{
	...
	int n1=10;
	double mb=param->mass_b+0.37; /* mb(kin) */
	double alphas_upsilon=alphas_running(mb,param->mass_top_pole,param->mass_b_pole,param);

	double t=0.;
	double int1=F2_a(t)/2.;
	double int2=F2_na(t)/2.;
	double int3=F2_nf(t)/2.;
...
{
	if(fabs(z-1.)<1.e-5) return 4.0859;
	if(fabs(z)<1.e-5) return 0.;
...
double Re_b(double z)
{
	...
	if(fabs(z-1.)<1.e-5) return 0.0316;
	if(fabs(z)<1.e-5) return 0.;
...
double C_BXlnu(struct parameters* param)
{
	...
	return g_rho*(0.849-0.92*delta_alphas+0.0596*delta_b-0.2237*(mc_3GeV-1.)-0.0167*param->mu_G2_bsg-0.203*param->rho_D3_bsg+0.004*param->rho_LS3_bsg); 	
}

/*----------------------------------------------------------------------*/
/*----------------------------------------------------------------------*/

double bsgamma_Ecut(double complex C0[], double complex C1[], double complex C2[], double complex Cp[], double mu, double mu_W, double E0, struct parameters* param)
/* computes the inclusive branching ratio of B -> Xs gamma for a given cut on the photon energy */
/* C0, C1, C2: respectively LO, NLO and NNLO contributions of the Wilson coefficients at scale mu=O(mb) */
/* E0: cut on the photon energy */
{
	int ie,je;	
	...
	double P0 = C0[7]*conj(C0[7]) + Cp[7]*conj(Cp[7]);
	double P1_1 = 2.*creal(C0[7]*conj(C1[7]));
	double P2_1 = C1[7]*conj(C1[7]) + 2.*creal(C0[7]*conj(C2[7]));
	...
	double z=pow(running_mass(param->mass_c,param->mass_c,param->mu_c_bsg,param->mass_top_pole,param->mass_b,param)/mb,2.); 
		
	double delta=1.-2.*E0/mb;
	...
	phi1[8][8]=phi88(delta,mb/param->mass_s);
	...
	{ 
		P1_2+=creal(C0[ie]*conj(C0[je])+Cp[ie]*conj(Cp[je]))*K1[ie][je];
		P2_3+=2.*creal(C0[ie]*conj(C1[je]))*K1[ie][je];
	}
	...
	double a,b,a3,b3,a4,b4;
	
	x1=C0[2]*conj(C0[2])+C0[1]*conj(C0[1])/36.-creal(C0[1]*conj(C0[2]))/3.;
	
	x2=9./8.*(-2.*4736./729.*creal(C0[2]*conj(C0[7]))+(1./3.*4736./729.+140./27.)*creal(C0[1]*conj(C0[7]))
	-32./9.*C0[7]*conj(C0[7])+32./27.*creal(C0[7]*conj(C0[8])));	
	...
	double P22rem_z0 = C0[2]*conj(C0[2])*K22rem + C0[1]*conj(C0[1])*K11rem + 2.*creal(C0[1]*conj(C0[2]))*K12rem + 2.* creal(C0[2]*conj(C0[7]))*K27rem + 2.*creal(C0[2]*conj(C0[8]))*K28rem + 2.*creal(C0[1]*conj(C0[7]))*K17rem + 2.*creal(C0[1]*conj(C0[8]))*K18rem + C0[7]*conj(C0[7])*K77rem + 2.*creal(C0[7]*conj(C0[8]))*K78rem + C0[8]*conj(C0[8])*K88rem;
	...
	double P22rem_z1 = C0[2]*conj(C0[2])*K22rem + C0[1]*conj(C0[1])*K11rem + 2.*creal(C0[1]*conj(C0[2]))*K12rem + 2.* creal(C0[2]*conj(C0[7]))*K27rem + 2.*creal(C0[2]*conj(C0[8]))*K28rem + 2.*creal(C0[1]*conj(C0[7]))*K17rem + 2.*creal(C0[1]*conj(C0[8]))*K18rem + C0[7]*conj(C0[7])*K77rem + 2.*creal(C0[7]*conj(C0[8]))*K78rem + C0[8]*conj(C0[8])*K88rem;
...
/* --------------------------------------------- */
	...
	double K77rem_z0=(K1[7][7]-4.*phi77(delta)+2./3.*Lb)*K1[7][7]-587708./729.-628./405.*pow(pi,4.)
	+32651./729.*pi*pi+428./27.*pi*pi*log(2.)+25150./81.*zeta3-448./9.*Lb*Lb+(80./9.*pi*pi-2524./9.)*Lb
	+512./27.*pi*alphas_upsilon+4.*phi77_2-8.*phi2_beta[7][7]/3./beta0;

	x5=C0[7]*conj(C0[7])*K77rem_z0;
...
#ifdef DEBUG
	printf("cas (c) : x1=%f\t x2=%f\t x3=%f\t x4=%f\t x5=%f\n\n",x1, x2, x3, x4, x5);
#endif

	double P2_2_rem3=x1*(Re_r1[2]*Re_r1[2]-(1666./243.)*(1666./243.)+Im_r1_2*Im_r1_2-(80./81.*pi*80./81.*pi))
	+x2*(Re_r2-(67454./6561.-124./729.*pi*pi))
	+x3*(Re_r1[2]-(-1666./243.))
	+x4*z*dRe_r1_2_dz
	+x5;
	...
	double complex Cem[9],C0w[11],C1w[11],C2w[11];
	...
	double Kt0=pow(eta_mu,4./23.)*(creal(C0w[7])+23./36.)-8./3.*(pow(eta_mu,4./23.)-pow(eta_mu,2./23.))*(creal(C0w[8])+1./3.);
	...
	double P_em=1./param->inv_alpha_em*(2.*creal(Cem[7]*conj(C0[7]))-C0[7]*conj(C0[7])*2.*alphas_mu*log(param->mass_W/mu)/pi)/alphas_mu;

#ifdef DEBUG
	for (ie=1;ie<=8;ie++) printf("C0[%d]=%f\t C1[%d]=%f\n",ie,creal(C0[ie]),ie,creal(C1[ie]));
	printf("C2[7]=%f\n\n",C2[7]);
#endif
...
{
	double E0=1.6;
	return bsgamma_Ecut(C0,C1,C2,Cp,mu,mu_W,E0,param);
}
/*---------------------------------------------------------------------*/
...
{
	double complex C0w[11],C1w[11],C2w[11],C0b[11],C1b[11],C2b[11],Cpb[11],CQpb[3];
