...
/* C0_spec and C1_spec: respectively LO and NLO contributions of the Wilson coefficients at scale mu_spec=O(sqrt(lambda_h*mb)) */
{
	...
	double T1=param->T1_BKstar;

	double lambda_B=param->lambda_Bp;
	lambda_B /= 1.+alphas_muspec/3./pi*log(pow(muspec,2.))*(1.-2.*1.4);
	
	double f_Kstar=param->f_Kstar_par;
	...
	a1_perp*=pow(eta,4./3.*(4.*1./2.)/(11.-2./3.*nf));
	a2_perp*=pow(eta,4./3.*(4.*(1./2.+1./3.))/(11.-2./3.*nf));
	...
	double H8a7= 4./3./N*pi*pi*param->f_B*f_Kstar_perp/T1/param->m_B/lambda_B*H8_perp(a1_perp,a2_perp);
	
	double complex H2a7= -2./3./N*pi*pi*param->f_B*f_Kstar_perp/T1/param->m_B/lambda_B*H2_perp(sc,a1_perp,a2_perp);
	...
	double complex a7c=(C[7]+Cp[7]) + alphas_mub/4./pi*4./3.*(C[2]*G2a7+C[8]*G8a7) + alphas_muspec/4./pi*4./3.*(C_spec[8]*H8a7+C_spec[2]*H2a7);
	...
	double complex b_d=12.*pi*pi*param->f_B*(-1./3.)/mass_b_mub/T1/a7c*(f_Kstar_perp/mass_b_mub*K1+f_Kstar*param->m_Kstar/6./lambda_B/param->m_B*K2d);
	
	double complex b_u=12.*pi*pi*param->f_B*(2./3.)/mass_b_mub/T1/a7c*(f_Kstar_perp/mass_b_mub*K1+f_Kstar*param->m_Kstar/6./lambda_B/param->m_B*K2u);


	double r1p=(8./3.*Cp[3]+4./3.*nf*(Cp[4]+Cp[6])-8.*(N*Cp[6]+Cp[5]))*F_perp(a1_perp,a2_perp)*log(mub/mu0);
	double r2p=(-44./3.*Cp[3]-4./3.*nf*(Cp[4]+Cp[6]))*log(mub/mu0);
		
	double complex K1p=-(Cp[6]+Cp[5]/3.)*Fperp + 4./9.*alphas_mub/4./pi*(pow(mass_b_mub/param->m_B,2.)*Cp[8]*Xperp-Cp[2]*((4./3.*log(param->mass_b_1S/mub)+2./3.)*Fperp-Gperp)+r1p); 	

	double complex K2up=lambda_u_lambda_c*(Cp[2]+Cp[1]/3.)+(Cp[4]+Cp[3]/3.)+4./9.*alphas_mub/4./pi*(Cp[2]*(4./3.*log(param->mass_b_1S/mub)+2./3.-Hperp)+r2p);
	
	double complex K2dp=(Cp[4]+Cp[3]/3.)+4./9.*alphas_mub/4./pi*(Cp[2]*(4./3.*log(param->mass_b_1S/mub)+2./3.-Hperp)+r2p);
		
	double complex bp_d=12.*pi*pi*param->f_B*(-1./3.)/mass_b_mub/T1/a7c*(f_Kstar_perp/mass_b_mub*K1p+f_Kstar*param->m_Kstar/6./lambda_B/param->m_B*K2dp);
	
	double complex bp_u=12.*pi*pi*param->f_B*(2./3.)/mass_b_mub/T1/a7c*(f_Kstar_perp/mass_b_mub*K1p+f_Kstar*param->m_Kstar/6./lambda_B/param->m_B*K2up);
	...
	printf("-----------------\n");
	for(ie=1;ie<=8;ie++) printf("C0[%d]=%f\t C1[%d]=%f\t C0spec[%d]=%f\n",ie,creal(C0[ie]),ie,creal(C1[ie]),ie,creal(C0_spec[ie]));
	printf("a7=%f+I*%f\n\n",creal(a7c),cimag(a7c));
#endif

	return creal(b_d-b_u)+creal(bp_d-bp_u);
}
...
{
	double complex C0b[11],C0spec[11],C1b[11],C1spec[11],Cpb[11],C0w[11],C1w[11],C2w[11],CQp[3];
	...
	Cprime_calculator(2,Cpb,CQp,mu_W,mu_b,&param);

	return delta0(C0b,C0spec,C1b,C1spec,Cpb,&param,mu_b,mu_spec,lambda_h);
}
