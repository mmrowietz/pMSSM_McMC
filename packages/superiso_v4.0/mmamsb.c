...
/*--------------------------------------------------------------------*/
/* Calculation of the observables, corresponding to a Mixed-Moduli AMSB point generated by ISAJET */
/*--------------------------------------------------------------------*/
...
int main(int argc,char** argv)
{
	...
	if(!test_file(ISAJET)) 
	{
		printf("\"%s\" absent. Please check the ISAJET path or comment \"#define USE_ISAJET\" in mmamsb.c\n",ISAJET);
		filesOK=0;
	}
#endif
	if(!filesOK) return 1;
...
#ifdef USE_ISAJET
	sprintf(name,"mmamsb_isajet%d.tmplha",getpid());
	isajet_mmamsb(alpha, m32, tanb, sgnmu, mtop, name);
	printf("MMAMSB - SLHA file generated by ISAJET\n\n");
	...
	sprintf(name,"rm mmamsb_isajet%d.tmplha",getpid());
	system(name);
#endif
